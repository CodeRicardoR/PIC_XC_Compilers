...
void ConfigPort_SPI(void){
    ...
    SSPSTATbits.SMP = 0;        //Data input sampled at the middle.
    SSPSTATbits.CKE = 1;        //tx on from active to idle
    SSPCON1bits.CKP = 0;        //LOW is idle for CLK
    SSPCON1bits.SSPM = 2;       //Freq = FOSC/64 = 614.4 KHz
    return;
}
